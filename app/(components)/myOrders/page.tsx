...
const MyOrdersPage: React.FC = () => {
  ...
  const handleReportProblem = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to report problem");
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      for (const key in supportBoxRef.current) {
        if (supportBoxRef.current[key] && !supportBoxRef.current[key]?.contains(target) && supportBoxOpen[key]) {
    ...
    <div className="container mx-auto px-6 py-10 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {orders.length === 0 ? (
          ...
          orders.map((order) => (
            <div key={order.id} className="p-6 bg-white rounded-lg shadow-lg hover:shadow-xl duration-300">
              ...
              <p className="text-gray-600"><span className="font-semibold">Delivery Date:</span> {order.deliveryDate ? new Date(order.deliveryDate).toLocaleDateString() : "N/A"}</p>
              <p className="text-gray-600"><span className="font-semibold">Delivery Partner:</span> {order.DeliveryPerson?.name || "N/A"}</p>
              ...
              <button
                className={`mt-4 w-full py-2 px-4 bg-blue-500 text-white font-semibold rounded-lg shadow-md hover:bg-blue-600 transition-colors duration-300
                  ${order.ReportedProblem?.length ? "block" : "hidden"}`}
              ...
              <button
                className={`mt-4 w-full py-2 px-4 bg-green-500 text-white font-semibold rounded-lg shadow-md hover:bg-green-600 transition-colors duration-300
                  ${order.ReportedProblem?.length ? "hidden" : ""}`}
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                  <div className="bg-white p-6 rounded-lg shadow-lg w-96">
                    ...
                    <button
                      ...
                      onClick={() => setReportProblemModalOpen((prev) => ({ ...prev, [order.id]: false }))}
                    >
                      Cancel
                    </button>
                  </div>
