...
const AdminOrdersPage = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch("/admin/allOrder"); // Admin-specific API endpoint
        if (!response.ok) {
  ...
  return (
    <div className="container mx-auto px-6 py-10">
      <h1 className="text-4xl font-bold text-center mb-12 text-gray-800">All Orders (Admin)</h1>
            ...
            >
              ...
              <div className="space-y-2">
                <p className="text-gray-600">
                  <span className="font-semibold">User:</span> {order?.user?.name || "Unknown"} ({order?.user?.email})
                </p>
                <p className="text-gray-600">
              ...
              <button className="mt-4 w-full py-2 px-4 bg-red-500 text-white font-semibold rounded-lg shadow-md hover:bg-red-600 transition-colors duration-300">
                Cancel Order
              </button>
            </div>
          ))
